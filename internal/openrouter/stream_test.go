...
func TestStreamTurnText(t *testing.T) {
	...
	msg, err := New(srv.URL, "k", nil).StreamTurn(context.Background(), baseTurnReq(),
	...
	if msg.API != "openai-completions" || msg.Provider != "openrouter" || msg.Model != "test/model" {
...
func TestStreamTurnUsageOnlyFinalChunk(t *testing.T) {
	...
	msg, err := New(srv.URL, "k", nil).StreamTurn(context.Background(), baseTurnReq(), nil, nil)
	if err != nil {
		t.Fatalf("StreamTurn: %v", err)
	}
...
func TestStreamTurnEmptyStream(t *testing.T) {
	...
	_, err := New(srv.URL, "k", nil).StreamTurn(context.Background(), baseTurnReq(), nil, nil)
...
func TestStreamTurnCancellation(t *testing.T) {
	...
	go func() {
		m, err := New(srv.URL, "k", nil).StreamTurn(ctx, baseTurnReq(), nil, nil)
		ch <- result{m, err}
	}()
