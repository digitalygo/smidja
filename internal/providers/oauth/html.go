...
func writeSuccess(w http.ResponseWriter, message string) {
	...
	fmt.Fprint(w, oauthPage("Authentication successful", "Authentication successful", message, ""))
}
...
func writeError(w http.ResponseWriter, status int, message string, details string) {
	...
	fmt.Fprint(w, oauthPage("Authentication failed", "Authentication failed", message, details))
}

func oauthPage(title, heading, message, details string) string {
	detailsBlock := ""
	if details != "" {
		detailsBlock = `<div class="details">` + html.EscapeString(details) + `</div>`
	}
	return `<!doctype html>
<html lang="en">
<head>
